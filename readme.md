# Vite + TypeScript + TailwindCSS + DaisyUI 快速启动模板
...
这是一个现代前端开发模板，集成了 Vite、TypeScript、TailwindCSS 和 DaisyUI，并配置了通过 GitHub Actions 实现的自动化部署流程。你只需要将代码推送到 `main` 分支，网站就会自动构建并发布到 GitHub Pages。
...
- **页面结构**: 修改 `index.html`。
- **页面样式**: 在 HTML 标签中使用 TailwindCSS 的类名，例如 `<h1 class="text-3xl font-bold">`。
- **页面逻辑**: 修改 `src/` 目录下的 TypeScript 文件。
